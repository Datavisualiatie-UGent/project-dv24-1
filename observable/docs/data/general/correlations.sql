select s2.commercial_name as station, count(s.train_id) as num_trains,
       avg(s.arrival_delay) / 60 as avg_arrival_delay, avg(s.departure_delay) / 60 as avg_departure_delay
